...
- Configuración: binario genérico. `src/config.rs` solo fija el repo central
  (`GeraldGlitch/ggupdater`); el manifest se resuelve con el `--app` recibido, así que un mismo
  binario sirve para todas las apps. El repo aloja `manifests/<app_id>.json`, `banners/` y los releases.
